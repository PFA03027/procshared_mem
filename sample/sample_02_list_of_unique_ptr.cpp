...
unsigned char sub_process_func( void )
{
	...
	sample_list_element           oup_msg = std::move( op_sl->front() );
	op_sl->front();

	return static_cast<unsigned char>( oup_msg->v_ );
}
...
int main( void )
{
	...
	msg_sample mm;
	mm.v_        = 43;
	auto oup_msg = ipsm::allocate_offset_unique<msg_sample>( sample_shared_memory.get_offset_malloc(), mm );
	p_que_list->push_back( std::move( oup_msg ) );
