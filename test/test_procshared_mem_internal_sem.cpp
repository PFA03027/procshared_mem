...
TEST( Test_semaphore_resource_handler, CanDefaultConstruct_CanDestruct )
{
	...
	ASSERT_NO_THROW( {
		// Act
		semaphore_resource_handler sut;
...
TEST( Test_semaphore_resource_handler, CanConstruct_CanDestruct )
{
	...
	ASSERT_NO_THROW( {
		// Act
		semaphore_resource_handler sut( p_shm_name, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP );
...
TEST( Test_semaphore_resource_handler, CanMoveConstruct )
{
	// Arrange
	sem_unlink( p_shm_name );
	semaphore_resource_handler sut_src( p_shm_name, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP );

	ASSERT_NO_THROW( {
		// Act
		semaphore_resource_handler sut = std::move( sut_src );
...
TEST( Test_semaphore_resource_handler, CanMoveAssignment )
{
	...
	// Act
	ASSERT_NO_THROW( {
		sut = semaphore_resource_handler( p_shm_name, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP );
	} );
...
TEST( Test_semaphore_resource_handler, CanSwap )
{
	// Arrange
	sem_unlink( p_shm_name );
	semaphore_resource_handler sut1;
	semaphore_resource_handler sut2( p_shm_name, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP );
...
TEST( Test_semaphore_resource_handler, CanDoUnlink )
{
	...
	sem_unlink( p_shm_name );
	semaphore_resource_handler sut( p_shm_name, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP );
	semaphore_resource_handler tmp_sem( p_shm_name, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP );
	...
	// Assert
	semaphore_resource_handler sut2;
	ASSERT_NO_THROW( {
		sut2 = semaphore_resource_handler( p_shm_name, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP );
	} );
