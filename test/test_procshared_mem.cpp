...
TEST( Test_procshared_mem, CanConstruct_CanDestruct )
{
	...
	// Act
	ASSERT_NO_THROW(
		procshared_mem shm_obj( p_shm_obj_name, 4096, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, [&test_flag]( void* p_mem, off_t len ) {
			if ( p_mem == nullptr ) {
				return;
			}
			if ( len < 4096 ) {
...
TEST( Test_procshared_mem, CanConstructDefer_CanDestruct )
{
	...
	procshared_mem::debug_force_cleanup( p_shm_obj_name );   // to remove ghost data
	bool           test_flag = false;
	procshared_mem shm_obj( p_shm_obj_name, 4096 - 16, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, procshared_mem_defer );
...
TEST( Test_procshared_mem, CanConstructDefer_Primary )
{
	...
	procshared_mem::debug_force_cleanup( p_shm_obj_name );   // to remove ghost data
	bool           test_flag = false;
	procshared_mem shm_obj( p_shm_obj_name, 4096, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, procshared_mem_defer );
...
TEST( Test_procshared_mem, CanConstruct_Secondary )
{
	...
	*( reinterpret_cast<int*>( shm_obj.get() ) ) = 12;
	...
	std::thread t1( std::move( task1 ), []() -> int {
		procshared_mem shm_obj_secondary( p_shm_obj_name, 4096, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP );
...
TEST( Test_procshared_mem, CanConstruct_Secondary_by_both )
{
	...
	*( reinterpret_cast<int*>( shm_obj.get() ) ) = 12;
	...
	// Act
	std::thread t1( std::move( task1 ), []() -> int {
		procshared_mem shm_obj_secondary_by_both( p_shm_obj_name, 4096, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, []( void* p_mem, off_t len ) {
