...
void offset_shared_ptr_ctrl_block_if::try_dispose_com( offset_ptr<offset_shared_ptr_ctrl_block_if>& p, long offset_shared_ptr_ctrl_block_if::ctrl_data::* mp_tcnt )
{
	if ( p == nullptr ) return;
	...
	if ( ( ac.ref().shrd_refc_ <= 0 ) && ( ac.ref().weak_refc_ <= 0 ) ) {
		ctrl_block_deleter_if* p_alloc_if = p->get_self_deleter();
		...
		{
			offset_shared_ptr_ctrl_block_if::ascer tmp( std::move( ac ) );
